...
# Función que simula un cliente enviando solicitudes filter_text
def client_task(num_requests):
    service = Pyro4.Proxy("PYRONAME:filter.service")
    pid = os.getpid()
    start_time = time.time()
    for i in range(num_requests):
        try:
            filtered = service.filter_text(f"This is a test from process {pid}_{i} text with some insults like dummy and idiot.")  # Filtrar texto
        except Exception as e:
            print(f"Error: {e}")
    end_time = time.time()
    duration = end_time - start_time
    print(f"Client completed {num_requests} requests in {duration:.2f} seconds")
    return duration
...
if __name__ == "__main__":
    ...
    # Añadir insultos a InsultService antes de la prueba
